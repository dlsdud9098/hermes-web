...
## 프로젝트 = 폴더 (Codex/Claude CLI 식)

프로젝트는 곧 폴더다. 레일의 `+` 로 폴더 절대경로를 열면 프로젝트가 생기고,
이름은 폴더명에서 자동으로 정해진다(별도 이름 입력 없음). 첫 실행 시에도 폴더를 열어야 시작한다.

프론트는 채팅마다 `hermes-web:cwd=<경로>` 마커를 함께 보내고, 플러그인이 매 턴
"이 폴더 기준으로 작업하라"는 지시를 주입한다 — 에이전트의 파일/터미널 툴이 그 폴더에서 동작.

소프트 바인딩(지시 기반)이라 하드 chroot 는 아니다. 경로는 **게이트웨이 호스트** 기준
절대경로이며, 폴더 접근은 서버측 에이전트가 한다(브라우저는 폴더에 접근하지 않음).
